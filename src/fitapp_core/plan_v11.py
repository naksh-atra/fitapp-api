...
def _extract_prescriptions_from_hits(hits: List[Dict[str, Any]], goal: str) -> Dict[str, Any]:
    ...
    for hit in hits:
        ...
        print(f"[Prescriptions] Checking doc {doc_id}, text length: {len(text)}")
        print(f"[Prescriptions] Text preview: {text[:200]}...")
        
        # Check for prescription content based on goal
        has_prescription_content = False
        
        if goal_lower == "hypertrophy":
            has_prescription_content = (
                "PROGRAMMING PRESCRIPTIONS" in text.upper() or 
                "PRESCRIPTIONS" in text.upper() or
                ("sets:" in text.lower() and "reps:" in text.lower() and "tempo:" in text.lower())
            )
        elif goal_lower == "strength":
            has_prescription_content = (
                "PROGRAMMING PRESCRIPTIONS" in text.upper() or 
                "PRESCRIPTIONS" in text.upper() or
                "STRENGTH TRAINING PRESCRIPTIONS" in text.upper() or
                ("1RM" in text or "RPE" in text or "RIR" in text)
            )
        elif goal_lower == "endurance":
            has_prescription_content = (
                "PROGRAMMING PRESCRIPTIONS" in text.upper() or 
                "PRESCRIPTIONS" in text.upper() or
                "ENDURANCE TRAINING PRESCRIPTIONS" in text.upper() or
                ("duration:" in text.lower() and "zone" in text.lower())
            )
        elif goal_lower == "fat_loss" or goal_lower == "fatloss":
            has_prescription_content = (
                "PROGRAMMING PRESCRIPTIONS" in text.upper() or 
                "PRESCRIPTIONS" in text.upper() or
                "FAT LOSS TRAINING PRESCRIPTIONS" in text.upper() or
                ("work_interval" in text.lower() and "rest_interval" in text.lower()) or
                "FL-PRSC" in text or  # Your prescription chunk IDs
                # Also accept research PDFs with specific HIIT guidance
                ("programmingrules" in text.lower() and "hiit" in text.lower()) or
                ("workoutgeneratorrules" in text.lower() and ("interval" in text.lower() or "hiit" in text.lower()))
            )
        
        print(f"[Prescriptions] has_prescription_content for {goal_lower}: {has_prescription_content}")
        
        if not has_prescription_content:
            continue
        
        prescriptions["found"] = True
        print(f"[Prescriptions] Found prescription data for {goal_lower} in doc {doc_id}, chunk {hit.get('id', 'unknown')}")
...
def _extract_fatloss_prescriptions(text: str) -> Dict[str, Any]:
    ...
    # Your PDFs use these exact patterns
    # Work interval: "work_interval 20-40 seconds"
    work_match = re.search(r"work_interval\s+(\d+)-(\d+)\s+seconds?", text, re.IGNORECASE)
    if work_match:
        # Use midpoint or lower bound
        presc["main"]["work_interval"] = f"{work_match.group(1)}s"
        print(f"[Fat Loss] Work interval: {presc['main']['work_interval']}")
    
    # Rest interval: "rest_interval 20-40 seconds"
    rest_match = re.search(r"rest_interval\s+(\d+)-(\d+)\s+seconds?", text, re.IGNORECASE)
    if rest_match:
        presc["main"]["rest_interval"] = f"{rest_match.group(1)}s"
        print(f"[Fat Loss] Rest interval: {presc['main']['rest_interval']}")
    
    # Rounds: "rounds 4-8 per circuit"
    rounds_match = re.search(r"rounds\s+(\d+)-(\d+)", text, re.IGNORECASE)
    if rounds_match:
        presc["main"]["rounds"] = f"{rounds_match.group(1)}-{rounds_match.group(2)}"
        print(f"[Fat Loss] Rounds: {presc['main']['rounds']}")
    
    # Total time: "total_time 15-30 minutes"
    total_match = re.search(r"total_time\s+(\d+)-(\d+)\s+minutes?", text, re.IGNORECASE)
    if total_match:
        presc["main"]["total_time"] = f"{total_match.group(1)}-{total_match.group(2)}min"
        print(f"[Fat Loss] Total time: {presc['main']['total_time']}")
