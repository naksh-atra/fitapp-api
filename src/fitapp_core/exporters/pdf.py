...
def _cell_lines(day_rows: List[dict], goal: str) -> List[str]:
    ...
    - Fat Loss: Movement | Work Interval | Rest Interval | Rounds | Total Time
    """
    lines: List[str] = []
    ...
    for section in order:
        for r in (row for row in day_rows if row.get("block_type") == section):
            ...
            if goal_lower == "hypertrophy":
                ...
                # Use dedicated fields instead of tempo_or_rest
                tempo = r.get("tempo") or ""
                rest = r.get("rest") or ""
                target = r.get("target_muscle") or ""
                ...
                sets = r.get("sets"); reps = r.get("reps")
                vol = f"{int(sets)}x{int(reps)}" if isinstance(sets, int) and isinstance(reps, int) else ""
                weight = r.get("weight_or_1rm_pct") or ""
                rpe = r.get("rpe_or_rir") or ""
                rest = r.get("rest") or ""
                parts = [movement, vol, weight, rpe, rest]
                line = " | ".join(p for p in parts if p)
                
            elif goal_lower == "endurance":
                dur = r.get("duration_or_reps") or r.get("duration") or ""
                zone = r.get("intensity_zone") or ""
                cadence = r.get("cadence_or_pace") or ""
                parts = [movement, dur, zone, cadence]
                line = " | ".join(p for p in parts if p)
                
            elif goal_lower == "fat_loss" or goal_lower == "fatloss":
                work = r.get("work_interval") or ""
                rest_int = r.get("rest_interval") or ""
                rounds = r.get("rounds")
                rounds_str = f"{int(rounds)} rounds" if isinstance(rounds, int) else (str(rounds) if rounds else "")
                total = r.get("total_time") or ""
                parts = [movement, work, rest_int, rounds_str, total]
                line = " | ".join(p for p in parts if p)
...
def build_pdf_v11(plan: dict) -> bytes:
    """Build PDF from plan dict and return as bytes."""
    ...
    if goal_lower == "hypertrophy":
        ...
        col_guide = "Columns: Movement | Work Interval | Rest Interval | Rounds | Total Time"
    else:
